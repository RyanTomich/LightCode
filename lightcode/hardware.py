...
def initilize_hardware(hardware):
    ...
    Hardware.intercon = {
        ...
        (Start, GPU): HardwareConnection(0, 0),
    }
    ...
    for hw_obj in hardware:
        ...
        if isinstance(hw_obj, GPU):
            Hardware.intercon.update(
                {
                    (SRAM, GPU): HardwareConnection(
                        sram.clock_period, SRAM_READ + LOCAL_WRITE + LOCAL_READ
                    ),
                    (GPU, SRAM): HardwareConnection(
                        sram.clock_period, LOCAL_WRITE + LOCAL_READ + SRAM_WRITE
                    ),
                }
            )
...
class TuringComplete(Hardware):
    def __init__(self, clock_speed):
        new_algs = {
            "add": HardwareAlgorithm(
                "add",
                {self: (all_elm, energy_per_cycle_func_gen(all_elm))},
            ),
            ...
            "mean": HardwareAlgorithm(
                "mean",
                {
                    self: (
                        self._mean_cycles,
                        energy_per_cycle_func_gen(all_elm),
                    )
                },
            ),
        ...
        for opp, alg in new_algs.items():
            if opp not in self.algs:
                self.algs[opp] = alg
...
class GPU(TuringComplete):
    def __init__(self, clock_speed, GPC, TPC_per_GPC, SM_per_TPC, fp32_CUDA_cores_per_SM, TC_per_SM):
        ...
        self.algs = {
            "matmul_gpu": HardwareAlgorithm(
                ...
                {
                    self: (
                        self._gpu_matmul_cycles,
                        self._gpu_matmul_energy,
                    )
                },
            ),
            "dense_gpu": HardwareAlgorithm(
                "dense",
                {
                    self: (
                        self._gpu_matmul_cycles,
    ...
    def _gpu_matmul_energy(self, i, o):
        num_dot_products = ten_elm(o[0])
        length_dot_products = i[0][-1]
        return num_dot_products * length_dot_products * self.mac_energy
