...
def tokenize_input_padding(prompt, tokenizer):
    return tokenizer(prompt, padding='max_length', truncation=True, max_length=15, return_tensors='pt')
...
def prefill_step(prompt, tokenizer):
    ...
    with torch.no_grad():
        outputs_pad = model(input_ids_pad, attention_mask=attention_mask, use_cache=True)
...
def decoder_step(last_token_id, attention_mask, past_key_values):
    ...
    with torch.no_grad():
        outputs = model(last_token_id, attention_mask=attention_mask, past_key_values=past_key_values, use_cache=True)
...
def generate(prompt, tokenizer, num_tokens=10):
    ...
    for i in range(num_tokens):
        ...
        logits, past_key_values = decoder_step(last_token_id, attention_mask, past_key_values)
        last_token_id = torch.argmax(logits[:, -1, :], dim=-1).item()
    ...
    decoded_prompt = tokenizer.decode(input_ids.squeeze().tolist(), skip_special_tokens=True)
...
def onnx_export_decoder(model):
    ...
    dummy_past_key_values = [(get_kv_cache(model, 10), get_kv_cache(model, 10)) for _ in range(model.config.num_hidden_layers)]
...
def get_profile_prefill_decoder():
    ...
    session = ort.InferenceSession("../models/prefill.onnx", sess_options=session_options)
    ...
    session = ort.InferenceSession("../models/decoder.onnx", sess_options=session_options)
